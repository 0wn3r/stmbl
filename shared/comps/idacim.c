    ...
    case 14:
      ...
      if(PIN(r_ok) > 0.0) {
        ...
        if(PIN(r_known) > 0.0) {
          ...
          printf("# chord cancels. idacim0.drop_slope carries it.\n");
          if(!(PIN(dc_volt) > 1.0)) {
            printf("</font><font color='red'># no bias was subtracted: idacim0.dc_volt reads %f.\n", PIN(dc_volt));
            printf("# add \"idacim0.dc_volt = hv0.dc_volt\" to the config -- without\n");
          ...
          printf("# phase reading, and set idacim0.r_known.</font>\n");
        }
      } else {
